...
//        AdaptiveExoplayer application = (AdaptiveExoplayer) getApplication();
        DownloadManager downloadManager = AdaptiveExoplayer.getInstance(this).getDownloadManager();
        DownloadNotificationHelper downloadNotificationHelper = AdaptiveExoplayer.getInstance(this).getDownloadNotificationHelper();
...
//            .buildProgressNotification(R.drawable.logo_notification,null,null,downloads);
    ...
    public int onStartCommand(@Nullable Intent intent, int flags, int startId) {
        ...
        AdaptiveExoplayer application = AdaptiveExoplayer.getInstance(this);
        ...
        if(intent != null){
            ...
            switch (Objects.requireNonNull(intent.getAction())){
                case Constants.EXO_DOWNLOAD_ACTION_PAUSE:
                    AdaptiveExoplayer.getInstance(this).getDownloadManager().addDownload(downloadTracker.getDownloadRequest(uri), Download.STATE_STOPPED);

                    break;
                case Constants.EXO_DOWNLOAD_ACTION_START:
                    AdaptiveExoplayer.getInstance(this).getDownloadManager().addDownload(downloadTracker.getDownloadRequest(uri), Download.STOP_REASON_NONE);

                    break;

                case Constants.EXO_DOWNLOAD_ACTION_CANCEL:
                    AdaptiveExoplayer.getInstance(this).getDownloadManager().removeDownload(downloadTracker.getDownloadRequest(uri).id);
